...
def find_candidate_matches(
    ...
    Args:
       close_matches:
           An np.ndarray of shape (tot_query_symbols, num_close_matches)
           as returned by find_close_matches(), indicating two close matches
           within the reference text for each symbol in the query documents.
       text:
           The SourcedText corresponding to the query and reference documents
           combined; needed for its `doc` member which can be assumed to be an
           np.ndarray.
       length_ratio:
           Indicates the maximum candidate-region length, which will be reduced
           if the matching reference document was shorter than this.
       num_candidates:
           The number of candidate regions to find for each query document.
    Returns:
